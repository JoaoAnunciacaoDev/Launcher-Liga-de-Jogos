use std::{collections::BTreeMap, fs, fs::File, io::{self, Read, Write}, path::{Path, PathBuf}, process::Command, sync::{atomic::{AtomicBool, Ordering}, Mutex}, time::Duration};
...
#[serde(rename_all = "camelCase")]
struct DownloadProgress { game_id: String, downloaded_bytes: u64, total_bytes: Option<u64> }
...
#[derive(Serialize)]
struct CatalogResponse { games: Vec<CatalogGame>, source: String, detail: Option<String> }
...
fn parse_catalog(contents: &str) -> Result<Vec<CatalogGame>, String> {
    let games: Vec<CatalogGame> = serde_json::from_str(contents).map_err(|error| format!("Catálogo inválido: {error}"))?;
    if games.is_empty() { return Err("O catálogo não contém jogos.".into()); }
    if games.iter().any(|game| game.id.is_empty() || game.title.is_empty() || game.cover_url.as_deref().is_some_and(|url| !url.starts_with("https://")) || game.builds.values().any(|build| !build.download_url.starts_with("https://") || build.executable.is_empty())) {
...
fn cached_or_bundled(cached_catalog: Option<Vec<CatalogGame>>, fallback_catalog: Vec<CatalogGame>, detail: String) -> CatalogResponse {
    match cached_catalog {
        Some(games) => CatalogResponse { games, source: "cache".into(), detail: Some(detail) },
        None => CatalogResponse { games: fallback_catalog, source: "bundled".into(), detail: Some(detail) },
    }
}
...
fn load_catalog_from_sources(app: tauri::AppHandle) -> Result<CatalogResponse, String> {
    ...
    let cached_catalog = cached_contents.as_deref().and_then(|contents| parse_catalog(contents).ok());
    let fallback_catalog = parse_catalog(BUNDLED_CATALOG)?;
    let client = reqwest::blocking::Client::builder().timeout(Duration::from_secs(5)).build().map_err(|error| error.to_string())?;
    let remote_contents = client.get(REMOTE_CATALOG_URL).send().and_then(|response| response.error_for_status()).and_then(|response| response.text());
    match remote_contents {
        Ok(contents) => match parse_catalog(&contents) {
            Ok(remote_catalog) => {
                if cached_contents.as_deref() != Some(contents.as_str()) { fs::write(cache_path, contents).map_err(|error| error.to_string())?; }
                Ok(CatalogResponse { games: remote_catalog, source: "remote".into(), detail: None })
            }
            Err(error) => Ok(cached_or_bundled(cached_catalog, fallback_catalog, error)),
        },
        Err(error) => Ok(cached_or_bundled(cached_catalog, fallback_catalog, error.to_string())),
    }
}
...
fn cover_directory(app: &tauri::AppHandle, game_id: &str) -> Result<PathBuf, String> {
    ...
    Ok(app.path().app_data_dir().map_err(|error| error.to_string())?.join("covers").join(game_id))
}
...
fn cover_as_data_url(cover_path: &Path, mime_path: &Path) -> Result<String, String> {
    ...
    Ok(format!("data:{};base64,{}", mime.trim(), STANDARD.encode(bytes)))
}

fn cache_cover(app: tauri::AppHandle, game_id: String, cover_url: String) -> Result<String, String> {
    if !cover_url.starts_with("https://") { return Err("O endereço da capa deve usar HTTPS.".into()); }
    ...
    if fs::read_to_string(&source_path).ok().as_deref() == Some(cover_url.as_str()) && cover_path.is_file() && mime_path.is_file() {
        return cover_as_data_url(&cover_path, &mime_path);
    }
    let response = reqwest::blocking::Client::new().get(&cover_url).send()
        .map_err(|error| format!("Não foi possível baixar a capa: {error}"))?
        .error_for_status().map_err(|error| format!("O servidor recusou a capa: {error}"))?;
    let mime = response.headers().get(reqwest::header::CONTENT_TYPE).and_then(|value| value.to_str().ok()).unwrap_or("application/octet-stream").split(';').next().unwrap_or("").trim().to_string();
    if !mime.starts_with("image/") { return Err("O arquivo da capa não é uma imagem válida.".into()); }
    let bytes = response.bytes().map_err(|error| format!("Não foi possível ler a capa: {error}"))?;
...
#[tauri::command]
async fn get_cached_cover(app: tauri::AppHandle, game_id: String, cover_url: String) -> Result<String, String> {
    tauri::async_runtime::spawn_blocking(move || cache_cover(app, game_id, cover_url)).await
...
fn download_and_install(app: tauri::AppHandle, game_id: String, download_url: String, executable: String) -> Result<Installation, String> {
    let installation = installation(&app, &game_id, &executable)?;
    if !download_url.starts_with("https://") { return Err("O endereço de download deve usar HTTPS.".into()); }
    ...
    loop {
        let bytes_read = response.read(&mut buffer).map_err(|error| format!("Erro durante o download: {error}"))?;
        if bytes_read == 0 { break; }
        archive_file.write_all(&buffer[..bytes_read]).map_err(|error| error.to_string())?;
        downloaded_bytes += bytes_read as u64;
        let _ = app.emit("download-progress", DownloadProgress { game_id: game_id.clone(), downloaded_bytes, total_bytes });
    }
    drop(archive_file);
    ...
    #[cfg(unix)]
    {
        ...
        let mut permissions = fs::metadata(executable_path).map_err(|error| error.to_string())?.permissions();
...
#[tauri::command]
async fn install_game(app: tauri::AppHandle, game_id: String, download_url: String, executable: String) -> Result<Installation, String> {
    tauri::async_runtime::spawn_blocking(move || download_and_install(app, game_id, download_url, executable))
        .await
...
async fn uninstall_game(app: tauri::AppHandle, game_id: String) -> Result<(), String> {
    tauri::async_runtime::spawn_blocking(move || {
        safe_game_id(&game_id)?;
        let app_data = app.path().app_data_dir().map_err(|error| error.to_string())?;
        for path in [app_data.join("games").join(&game_id), app_data.join("covers").join(&game_id)] {
            if path.exists() { fs::remove_dir_all(path).map_err(|error| error.to_string())?; }
        }
        Ok(())
    }).await.map_err(|error| format!("A desinstalação foi interrompida: {error}"))?
}
...
pub fn run() {
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![current_platform, load_catalog, get_cached_cover, get_installation, install_game, uninstall_game, launch_game, exit_launcher])
